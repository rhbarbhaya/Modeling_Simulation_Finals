...
for i in range(100):
    ...
    env.run(until = 1000)
    Team_A_Cars.append(len(Revenue_A))
    Team_A_Revenue.append(Team_A_Cars[-1]*Revenue)
    Team_B_Cars.append(len(Revenue_B1) + len(Revenue_B2) + len(Revenue_B3) + len(Revenue_B4))
    Team_B_Revenue.append(Team_B_Cars[-1]*Revenue)
    Team_C_Cars.append(len(Revenue_C1) + len(Revenue_C2))
    Team_C_Revenue.append(Team_C_Cars[-1]*Revenue)
    Team_D_Cars.append(len(Revenue_D))
    Team_D_Revenue.append(Team_D_Cars[-1]*Revenue)

Overview = pd.DataFrame([Team_A_Cars, Team_A_Revenue, Team_B_Cars, Team_B_Revenue, Team_C_Cars, Team_C_Revenue, Team_D_Cars, Team_D_Revenue]).transpose()
Overview.columns = ["Team_A_Cars", "Team_A_Revenue", "Team_B_Cars", "Team_B_Revenue", "Team_C_Cars", "Team_C_Revenue", "Team_D_Cars", "Team_D_Revenue"]

Overview.to_csv("D:\Personal Documents\Rushabh's Documents\Stevens\Engineering Management\Rushabh\Subjects\SYS 611 Simulation & Modeling\Finals\Model.csv", sep = ",", encoding = "utf-8")